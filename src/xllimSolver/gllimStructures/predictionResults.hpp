...
{
    mat mean;      // The mean of the full GMM which stands for the prediction (D, N_obs)
    cube variance; // The variance of the prediction (D, D, N_obs)
    mat weights;   // The weights of the components of the GMM (N_obs, K)
    cube means;    // The means of each component in the GMM (D, N_obs, K)
    cube covs;     // The covariance matrices of each component in the GMM (D, D, K). The covariance is indenpendent from the observation.

    FullGMMResult(unsigned N_obs, unsigned D, unsigned K) : mean(D, N_obs), variance(D, D, N_obs), weights(N_obs, K), means(D, N_obs, K), covs(D, D, K) {}
};

struct MergedGMMResult
{
    mat mean;               // The mean of the merged GMM which stands for the prediction (D, N_obs)
    cube variance;          // The variance of the prediction from the merged GMM (D, D, N_obs)
    mat weights;            // The weights of the merged GMM with shape (N_obs, K_merged)
    cube means;             // The means of the merged GMM with shape (D, N_obs, K_merged). It corresponds to the centers that stands for the predictions
    ...
    MergedGMMResult(unsigned N_obs, unsigned D, unsigned K_merged) : mean(D, N_obs), variance(D, D, N_obs), weights(N_obs, K_merged), means(D, N_obs, K_merged), covs(N_obs, cube(D, D, K_merged))
    {
