...
namespace DataGeneration{
    class StatModel{
    public:
        virtual void gen_data(std::shared_ptr<FunctionalModel> functionalModel, int n, double *x, double *y, unsigned seed) {
            int dimension_L = functionalModel->get_L_dimension();
            int dimension_D = functionalModel->get_D_dimension();

            std::tuple<mat, mat> data = gen_data(functionalModel, n, seed);
        ...
        virtual std::tuple<mat, mat> gen_data(std::shared_ptr<FunctionalModel> functionalModel, int n,unsigned seed) = 0;
