...
void GLLiMLearning<T, U>::importModel(GLLiM &gllim) {
    ...
    gllim_parameters = GLLiMParameters<T,U>(gllim.D, gllim.L, gllim.K);
    ...
    for(unsigned k=0; k<gllim.K; k++){
        ...
        for(unsigned l=0; l<gllim.L; l++){
            ...
            for(unsigned l2=0; l2<gllim.L; l2++){
                gllim_parameters->Gamma(l2,l) = gllim.Gamma[l2 + l*gllim.L + k*gllim.L*gllim.L];
            }
        }
        ...
        for(unsigned d=0; d<gllim.D; d++){
            ...
            for(unsigned d2=0; d2<gllim.D; d2++){
                gllim_parameters->Sigma(d2,d) = gllim.Sigma[d2 + d*gllim.D + k*gllim.D*gllim.D];
            }
            for(unsigned l=0; l<gllim.L; l++){
...
GLLiMParameters<FullCovariance,FullCovariance> GLLiMLearning<T, U>::inverse(GLLiMParameters<T,U> &gllim_direct) {
    ...
    for(unsigned k=0; k<gllim_direct.K; k++){
        if(gllim_direct.Pi(k) != 0){
            ...
            gllim_inv.Sigma[i] = FullCovariance(gamma_inv + gllim_direct.A.slice(i).t() * sigma_inv * gllim_direct.A.slice(i)).inv();
            gllim_inv.A.slice(i) = gllim_inv.Sigma[i] * gllim_direct.A.slice(i).t() * sigma_inv;
            gllim_inv.B.col(i) = gllim_inv.Sigma[i] * (gamma_inv * vec(gllim_direct.C.col(i)) - gllim_direct.A.slice(i).t() * sigma_inv * gllim_direct.B.col(i));
        }
    }
