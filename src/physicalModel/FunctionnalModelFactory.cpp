...
std::shared_ptr<FunctionnalModel> FunctionnalModelFactory::getModel(std::string type,const double *data, int row_size, int col_size) {
    if(type == "hapke02"){
        return std::shared_ptr<FunctionnalModel> (new Hapke02Model(data,row_size,col_size));
    }else {
        return std::shared_ptr<FunctionnalModel> (new Hapke93Model(data,row_size,col_size));
    }
}
